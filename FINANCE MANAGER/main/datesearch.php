...
<tbody>
	<?php
		...
		$fromdate = $_POST['from'];
		$todate = $_POST['to'];		
		$result = $db->prepare("SELECT * FROM transaction WHERE date BETWEEN '$fromdate' AND '$todate' ORDER BY id DESC");
		$result->execute();
	...
	<tr class="record">
		...
		<?php
		include('connect.php');		
		$result = $db->prepare("SELECT SUM(ft) as total FROM transaction WHERE date BETWEEN '$fromdate' AND '$todate'");
